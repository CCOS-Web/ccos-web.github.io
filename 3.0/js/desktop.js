...
function addwin(url, title, spec) {
	...
	if (closeSet.delete(`title_${url}`)) {
		...
		var fr = document.getElementById(`fram_${url}`);
		fr.src = fr.src;	// refresh
		return 0;
	}
	...
	i2.style.top = "0px";
	i2.style.left = "0px";
	i2.classList = "window_title centre";
	i2.dataset.lh = "437px";
	i2.dataset.lw = "700px";

	let i22 = document.createElement("font");
	i22.size = "5";
	i22.classList = "window_title_font secc";
	i22.innerHTML = title;
	...
	function onMousemove(event) {
		if (activeBorder) {
			var id = activeBorderEvent.target.id.substring(4, activeBorderEvent.target.id.length)
			var idt = Number(activeBorderEvent.target.id.substring(2, 3));
			// console.log(activeBorderEvent)
